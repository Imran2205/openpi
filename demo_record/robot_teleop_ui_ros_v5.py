...
class RobotROSNode(Node):
    ...
    def __init__(self, camera_node):
        ...
        # Integrated command base pose (x, y, theta) from the published base velocity commands.
        # This is recorded alongside action_command to avoid losing short velocity pulses when
        # samples are throttled to recorder FPS.
        self.command_base_pose_xytheta = None
        self.command_base_pose_last_wall_time = None
    ...
    def get_measured_base_pose_xytheta(self):
        """Latest measured base pose (x, y, theta) from odom."""
        if self.odom is None:
            return None
        try:
            pos = self.odom.pose.pose.position
            ori = self.odom.pose.pose.orientation  # xyzw
            theta = math.atan2(
                2.0 * (ori.w * ori.z + ori.x * ori.y),
                1.0 - 2.0 * (ori.y * ori.y + ori.z * ori.z),
            )
            return [float(pos.x), float(pos.y), float(theta)]
        except Exception:
            return None

    def get_command_base_pose_xytheta(self):
        """Latest integrated command base pose (x, y, theta)."""
        if self.command_base_pose_xytheta is not None:
            return list(self.command_base_pose_xytheta)
        return self.get_measured_base_pose_xytheta()

    def _ensure_command_base_pose_initialized(self, now_wall_time: float):
        if self.command_base_pose_xytheta is None:
            pose = self.get_measured_base_pose_xytheta()
            if pose is not None:
                self.command_base_pose_xytheta = list(pose)
    ...
    def _integrate_command_base_pose_until(self, now_wall_time: float):
        """Integrate command base pose using the *previously published* base velocity command."""
        self._ensure_command_base_pose_initialized(now_wall_time)
        if self.command_base_pose_xytheta is None:
            return
        if self.command_base_pose_last_wall_time is None:
            self.command_base_pose_last_wall_time = float(now_wall_time)
        ...
        if dt <= 0.0:
            self.command_base_pose_last_wall_time = float(now_wall_time)
            return
        # Guard against timer stalls causing one huge integration jump.
        dt = min(dt, 1.0)
    ...
    def publish_commands(self):
        ...
        if self.published_qpos is None or len(self.published_qpos) != len(self.qpos):
            self.published_qpos = list(self.qpos)
        # Integrate command pose using the command that has been active since the previous publish.
        self._integrate_command_base_pose_until(now_wall)
    ...
    def get_sensor_snapshot(self):
        ...
        sensors = {
            ...
            "observation.base_pose_xytheta": self.get_measured_base_pose_xytheta(),
            "observation.command_base_pose_xytheta": self.get_command_base_pose_xytheta(),
