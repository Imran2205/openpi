...
DEMO_RECORD_FPS = 10  # record images + synchronized telemetry at ~10 Hz
...
class LeRobotStyleRecorder:
    ...
    def start(self, dataset_root: str, prompt: str):
        ...
        videos_dir = self._ensure(root / "videos" / self.chunk_name)
        ...
        self.paths = {
            ...
            "head_rgb_video": self._ensure(videos_dir / "observation.images.head_rgb") / f"{self.episode_name}.mp4",
            "wrist_rgb_video": self._ensure(videos_dir / "observation.images.wrist_rgb") / f"{self.episode_name}.mp4",
            "head_depth_video": self._ensure(videos_dir / "observation.images.head_depth") / f"{self.episode_name}.mp4",
            "wrist_depth_video": self._ensure(videos_dir / "observation.images.wrist_depth") / f"{self.episode_name}.mp4",
    ...
    def _process_step(self, sample: dict):
        ...
        if head_rgb is not None:
            if "head_rgb" not in self.writers:
                self._create_writer("head_rgb", self.paths["head_rgb_video"], head_rgb)
            self.writers["head_rgb"].write(cv2.cvtColor(head_rgb, cv2.COLOR_RGB2BGR))

        if wrist_rgb is not None:
            if "wrist_rgb" not in self.writers:
                self._create_writer("wrist_rgb", self.paths["wrist_rgb_video"], wrist_rgb)
            self.writers["wrist_rgb"].write(cv2.cvtColor(wrist_rgb, cv2.COLOR_RGB2BGR))

        head_depth_png_rel = None
        if head_depth is not None:
            head_depth_rgb = self._depth_preview_rgb(head_depth)
            if "head_depth" not in self.writers:
                self._create_writer("head_depth", self.paths["head_depth_video"], head_depth_rgb)
            self.writers["head_depth"].write(cv2.cvtColor(head_depth_rgb, cv2.COLOR_RGB2BGR))
        ...
        if wrist_depth is not None:
            ...
            if "wrist_depth" not in self.writers:
                self._create_writer("wrist_depth", self.paths["wrist_depth_video"], wrist_depth_rgb)
            self.writers["wrist_depth"].write(cv2.cvtColor(wrist_depth_rgb, cv2.COLOR_RGB2BGR))
        ...
        row = {
            ...
            "observation.images.head_rgb": self._rel(self.paths["head_rgb_video"]),
            "observation.images.wrist_rgb": self._rel(self.paths["wrist_rgb_video"]),
            "observation.images.head_depth": self._rel(self.paths["head_depth_video"]),
            "observation.images.wrist_depth": self._rel(self.paths["wrist_depth_video"]),
    ...
    def stop(self):
        ...
        ep_meta = {
            ...
            "data_path": data_rel,
            "videos": {
                "head_rgb": self._rel(self.paths["head_rgb_video"]),
                "wrist_rgb": self._rel(self.paths["wrist_rgb_video"]),
                "head_depth": self._rel(self.paths["head_depth_video"]),
                "wrist_depth": self._rel(self.paths["wrist_depth_video"]),
            },
...
class RobotTeleopUI(QMainWindow):
    ...
    def _build_record_sample(self):
        ...
        command_qpos = list(self.ros_node.qpos) if self.ros_node.qpos is not None else []
        ...
        def run():
            try:
                ...
                if abs(rotation) > 0.02:
                    ...
                    time.sleep(0.1)
                    self.ros_node.qpos[9] = 0.0
                    time.sleep(max(1.0, abs(rotation) * 3.0))
